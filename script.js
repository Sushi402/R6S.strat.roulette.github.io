const attackerStrats = [
    ...
    "<strong>007</strong> The enemy defender with the highest score will become the target, and all attackers will only be able to kill that defender until they have died, in which case the next highest score will become the new target.",
    ...
    "<strong>Pay to Win</strong> Players can only select operators with the \"Black Ice\" skin on their primary and secondary.",
    "<strong>The Speed-Force</strong> All attackers can only play 3 speeds.",
    ...
    "<strong>Seal Team Six</strong> Every player brings smokes and flashes to throw into and push site.",
    "<strong>Knives Out</strong> All kills must be exclusively done by melees or knives.",
    "<strong>Butler, PA</strong> The lowest score on the team is appointed as \"President\" and must not die under any circumstances. If they do die, however, the rest of the team must surrender the round to the defenders."
];
...
const defenderStrats = [
    ...
    "<strong>A Quiet Place</strong> No words can be spoken in the duration of the round.",
    ...
    "<strong>Death From Afar</strong> Open up as many lines of sight into the site as possible, all defenders moving to a place outside the site and shooting from a distance.",
...
function getRandomStrat(strats) {
    let strat = strats[Math.floor(Math.random() * strats.length)];
    let [title, description] = strat.split('</strong>');
    return `<div class="strat-title">${title}</strong></div><div class="strat-description">${description}</div>`;
}

document.getElementById('attackerBtn').addEventListener('click', function() {
    document.getElementById('strategyOutput').innerHTML = getRandomStrat(attackerStrats);
});

document.getElementById('defenderBtn').addEventListener('click', function() {
    document.getElementById('strategyOutput').innerHTML = getRandomStrat(defenderStrats);
});
